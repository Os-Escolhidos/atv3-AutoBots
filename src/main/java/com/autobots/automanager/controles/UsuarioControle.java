...
public class UsuarioControle {
  ...
  @DeleteMapping("/excluir/{idCli}")
  public ResponseEntity<?> DeletarUsuario(@PathVariable Long idCli){
	  ...
	  if(user != null) {
		  ...
		  return new ResponseEntity<>("Excluído com sucesso", HttpStatus.ACCEPTED);
	  }else {
            ...
            if (EmailExistente == true) {
              ...
              if (cadastro.getPerfis().toString().contains("FORNECEDOR")) {
                if (cadastro.getMercadorias().isEmpty()) {
                  return new ResponseEntity<>(
                    "Fornecedor encontrado está sem mercadoria, por favor insira um",
                    HttpStatus.NOT_ACCEPTABLE
                  );
                } else {
